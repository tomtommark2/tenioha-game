...
### 2026-09-19 イラスト単語帳の無料100語（2026.0919.1923）

- イラスト単語帳だけ無料の固定100語、プレミアム全674語。一覧・出題・復習・保存済みデッキ・直接選択で同じ判定を使い、期限切れでも学習履歴を削除しない。
- 無料対象は `js/word_illustrations.js` の固定リスト（juniorの既存20語を含む100語）。追加登録で対象を入れ替えない。通常学習のイラスト・他の単語帳の利用範囲・1日8分の制限・購入判定は維持する。
- 目的別単語帳の有料制限等の未公開試作は含めない。Firebase・決済・復習配点は変更なし。静的公開のためアセット自体へのアクセス制御ではなく、アプリ内の利用範囲を制御する。
- 公開前確認：HTML・版番号・復習キー同期、単体32件・Functions18件・ブラウザー152件成功。モバイル関連8件と無料100語画面2件成功。320／390／1280pxで無料100語の全画像・有料674語・期限切れ・枠内表示を確認。

### 2026-09-19 無料版を1日8分へ統一（2026.0919.1904）

- 実際の制限を480秒に変更し、上部タイマー・制限画面・翌日リセット案内・購入案内を「1日8分」に統一。
- 旧10分版の当日消費時間と保存キーを維持。日本時間の日付変更による回復、購入済み利用者の時間制限解除は従来通り。
- 目的別単語帳の有料制限・イラスト単語帳20語制限等の未公開試作は含めない。今回公開済みの674語・647画像も維持。Firebase・決済処理は変更しない。

### 2026-09-16 読み上げ設定（2026.0916.1322）

- 公開対象は上部音声アイコン、自動読み上げON/OFF、専用音量、試聴、端末保存。音声選択順位は従来どおり、ボイス変更UIなし。320px幅でも無料タイマーと干渉しない配置と、有料時にタイマー枠を残さない表示を確認する。
- 公開中のmainから分離した `tmp/speech-release-20260916` で検証。未公開の8分制限・単語帳アクセス制限・その他の作業は含めず、無料版10分を維持。Firebaseの変更・デプロイなし。
- `test:e2e:safe` 成功（単体31件・Functions18件・ブラウザ145件）。分離環境のFunctions依存不足を解消して全手順を再実行した。実機TTSの音質は未試聴。
