...
- 設定は出題バランス→完璧の判定基準→復習タイミング→出題範囲の順。最下部に「カードの学習状況を表示」を置く。既存の判定・保存処理は共通で使う。
...
### カードの学習状況の表示（2026-09-24）

- 出題・復習設定の最下部で、カード右上の正答率と下部の出題理由（苦手／得意／完璧）をまとめて表示／非表示にする。初期値は表示。上部の分類ボタン・設定内の判定基準は対象外。
