...
- 2026-09-11：case〜coverの新規50画像を登録（color／colour共用で51語）。累計151語・147画像。[登録記録](experiments/pictogram-006-2026-09-11/registration.json)。SVG変換は量産工程に使わず内蔵画像生成を継続し、既存の語義・学習キー・原画は保持。制作元・未採用画像はローカル、配信には可逆WebPを使用する。公開状態は `operations.md`、次の制作語はcow。
...
- 表示切替で画面を上下させないため、意味カードは表裏を同じグリッド領域へ置き、未回答の意味は `visibility: hidden` で領域だけ確保する。「前の絵」も非表示時の領域を保持する。キャラ／画像は元から同じ固定枠を使う。同じイラストが表示済みなら意味カード操作や再表示で画像を消去・再生成しない。`tests/illustration-layout.spec.js` で位置・高さ、`tests/illustrated-wordbook.spec.js` で画像要素の維持と採点を検証する。
...
- 2026-09-10：既存100語・97画像すべてをピクトグラム寄りの新版へローカル差替え済み（新規語の追加ではない）。共用表記はairplane/aeroplane、bicycle/bike、cafe/café。原画・旧画像・語義・学習キーを保持し、配信WebPは寸法・可視RGBA不変。最新の残り27枚は全件80／100px目視・透過・スマホ幅を確認。カメラは透過欠損の初版と格子背景の補修版を不採用として再生成。[最新の新旧比較](experiments/pictogram-005-2026-09-10/index.html)／[第1回対応](experiments/noun-production/pictogram-rollout-01/sources.json)／[第2回対応](experiments/noun-production/pictogram-rollout-02/sources.json)／[第3回対応](experiments/noun-production/pictogram-rollout-03/sources.json)／[最新の可逆変換](experiments/noun-production/pictogram-rollout-03/encoding.json)。公開は明示依頼時のみ。
