...
const registration = require('../docs/experiments/pictogram-009-2026-09-13/registration.json');
const prompts = require('../docs/experiments/pictogram-009-2026-09-13/prompts.json');
const encoding = require('../docs/experiments/noun-production/pictogram-009/encoding.json');

test('通常生成の追加50画像・53語は未制作順を守り、既存257語を保持する', () => {
  ...
  expect(illustrations).toHaveLength(310);
  expect(new Set(illustrations.map(e => e.src)).size).toBe(297);
  const previous = illustrations.slice(0, 257);
  expect(crypto.createHash('sha256').update(JSON.stringify(previous)).digest('hex')).toBe(registration.previousManifestSha256);
  expect(illustrations.slice(257)).toEqual(registration.entries);
  ...
  expect(buildQueue({ ...input, illustrations }).entries.find(e => e.status === 'pending').word).toBe('jacket');
});
