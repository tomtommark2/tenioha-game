...
const registration = require('../docs/experiments/pictogram-008-2026-09-12/registration.json');
const prompts = require('../docs/experiments/pictogram-008-2026-09-12/prompts.json');
const encoding = require('../docs/experiments/noun-production/pictogram-008/encoding.json');

test('通常生成の追加50画像・51語は未制作順を守り、既存206語を保持する', () => {
  ...
  expect(illustrations).toHaveLength(257);
  expect(new Set(illustrations.map(e => e.src)).size).toBe(247);
  const previous = illustrations.slice(0, 206);
  expect(crypto.createHash('sha256').update(JSON.stringify(previous)).digest('hex')).toBe(registration.previousManifestSha256);
  expect(illustrations.slice(206)).toEqual(registration.entries);
  ...
  expect(buildQueue({ ...input, illustrations }).entries.find(e => e.status === 'pending').word).toBe('grandma');
});
