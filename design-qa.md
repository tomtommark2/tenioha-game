...
- 開発環境で関連13件、モバイルモーダル80件が通過。公開候補 `2026.0924.2122` の分離環境でもChromium／WebKitの関連26件が通過。再読込後の保持、回答記録、Undo、保存失敗時の復帰、320px幅とキーボード操作を確認。HTML／バージョン同期・JS構文・差分検査も通過。
- 実際のユーザー保存データは変更していない。実機Safariは未確認。
