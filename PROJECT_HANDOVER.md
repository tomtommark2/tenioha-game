...
## 4. 現在のステータス (v2.15 - 2026/01/13)
### 実装済み機能
*   **"無限プレミアム"バグ修正 (v2.15):** お試し版ユーザーを「旧課金ユーザー」と誤認して無期限ライセンスを付与してしまうレガシー移行ロジックを削除。
*   **PWAインストール特典のコード化 (v2.14):** アプリインストール時に自動付与していた特典を廃止し、プロモコード `app` （1日間）の案内ダイアログに変更。
