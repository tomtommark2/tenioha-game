...
## 4. 現在のステータス (v2.30 - 2026/01/15)
### 実装済み機能
*   **課金UIの改善 (v2.30):** 購入モーダルのデザインを修正。Note記事へのリンクを独立したボタン化し、ログイン推奨の警告メッセージを追加。
*   **Stripe Webhook連携 (v2.29):**
    *   **サーバー処理:** Firebase Cloud Functions にWebHook (`stripeWebhook`) を実装。Stripe決済完了時にFirestoreのユーザーデータをサーバーサイドで安全に更新するように変更。
    *   **クライアント処理:** 購入リンクに `?client_reference_id=ユーザーID` を動的に付与し、決済情報とユーザーを確実に紐付けるように改修。
*   **ランキング読み込み最適化 (v2.28):** クライアント側でランキングデータを5分間キャッシュするロジックを導入し、FirestoreのReadコストを劇的に削減。
*   **キルスイッチ動作検証完了 (v2.27):** アップデート要求画面の表示および「更新ボタン」による正常なリロード（キャッシュ削除含む）を確認済み。
    *   *Note:* 今後は `config/app_settings` の `min_required_version` を操作することで、任意のバージョン未満のユーザーをブロック可能です。
*   **更新ループ修正 (v2.26):** 強制アップデート時に、Service Workerの登録解除だけでなく、キャッシュストレージ (`caches.delete`) も明示的に全削除するように変更。
